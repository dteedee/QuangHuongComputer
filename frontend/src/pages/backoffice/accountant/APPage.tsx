                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="px-6 py-4 space-y-4">
                            <div className="bg-gray-50 p-4 rounded-xl space-y-2">
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500 font-bold text-xs uppercase">Mã hóa đơn</span>
                                    <span className="font-semibold text-gray-950 font-mono">{invoice.invoiceNumber}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500 font-bold text-xs uppercase">Còn nợ</span>
                                    <span className="font-semibold text-accent text-xl">{formatCurrency(invoice.outstandingAmount)}</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl focus:ring-0 focus:border-accent font-semibold text-xl text-accent"
                                    placeholder="0"
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
                            <button
                                type="button"
                                onClick={onClose}
                                className="px-5 py-2.5 text-gray-500 text-xs text-slate-500 hover:text-gray-900"
                                disabled={isSubmitting}
                            >
                                Hủy
                            </button>
                            <button
                                type="submit"
                                className="px-6 py-2.5 bg-accent text-white text-xs text-slate-500 rounded-xl shadow-lg hover:bg-accent-hover transition-all disabled:opacity-50"
                    ...
                    <form onSubmit={handleSubmit(handleFormSubmit)}>
                        <div className="px-8 py-6 space-y-5 max-h-[60vh] overflow-y-auto">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs text-slate-400 mb-2 block">Mã NCC *</label>
                                    <input
                                        type="text"
                                        {...register('supplierId')}
                                ...
                                <div>
                                    <label className="text-xs text-slate-400 mb-2 block">Hạn thanh toán</label>
                                    <input
                                        type="date"
                                        {...register('dueDate')}
                                        className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-100 rounded-xl focus:ring-0 focus:border-accent font-medium text-sm"
                                ...
                                <div>
                                    <label className="text-xs text-slate-400 mb-2 block">VAT mặc định (%)</label>
                                    <input
                                        type="number"
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <label className="text-xs text-slate-400">Danh sách hàng hóa</label>
                                    <button
                                        type="button"
                                        onClick={() => append({ description: '', quantity: 1, unitPrice: 0, vatRate: 10 })}
                                        className="px-3 py-1.5 bg-gray-100 text-gray-600 text-[9px] font-medium rounded-lg hover:bg-gray-200 flex items-center gap-1"
                                    >
                            ...
                            <div className="bg-gray-950 text-white p-6 rounded-xl space-y-3">
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-400 uppercase text-xs font-semibold">VAT</span>
                                ...
                                <div className="flex justify-between text-lg pt-3 border-t border-white/10">
                                    ...
                                    <span className="font-semibold text-accent text-2xl">{formatCurrency(total)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="px-8 py-6 bg-gray-50 border-t flex justify-end gap-4">
                            <button
                                type="button"
                                onClick={handleClose}
                                className="px-6 py-3 text-gray-500 text-xs text-slate-500 hover:text-gray-950"
                                disabled={isSubmitting}
                            >
                                Hủy
                            </button>
...
export const APPage = () => {
    ...
    const getStatusBadge = (status: InvoiceStatus) => {
        switch (status) {
            case 'Draft':
                return <span className="px-3 py-1 bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold">Bản nháp</span>;
            case 'Issued':
                return <span className="px-3 py-1 bg-amber-50 text-amber-600 rounded-lg text-xs font-semibold">Chờ xử lý</span>;
            case 'Paid':
                return <span className="px-3 py-1 bg-green-50 text-green-600 rounded-lg text-xs font-semibold">Đã thanh toán</span>;
            case 'Overdue':
                return <span className="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-semibold">Quá hạn</span>;
            case 'Cancelled':
                return <span className="px-3 py-1 bg-gray-50 text-gray-400 rounded-lg text-xs font-semibold">Đã hủy</span>;
            default:
                return null;
        }
    };
    ...
    const columns: Column<APInvoice>[] = [
        {
            ...
            label: 'Mã hóa đơn',
            sortable: true,
            render: (item) => <span className="font-semibold text-accent font-mono">{item.invoiceNumber}</span>,
        },
        {
            key: 'supplierId',
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="premium-card p-6">
                <h3 className="text-sm font-semibold text-gray-700 uppercase mb-4">Lọc theo trạng thái</h3>
                <div className="flex flex-wrap gap-3">
                    {statusFilters.map((filter) => (
                        <button
                            key={filter.key}
                            onClick={() => setSelectedStatus(filter.key)}
                            className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${selectedStatus === filter.key
                            ...
                            <div className="flex gap-2 justify-end">
                                {(item.status === 'Issued' || item.status === 'Overdue') && item.outstandingAmount > 0 && (
                                    <button
                                        ...
                                        className="px-6 py-2.5 bg-gray-950 text-white text-[9px] font-medium rounded-xl hover:bg-accent transition-all active:scale-95 shadow-lg shadow-gray-200"
                                    >
                                        Thanh toán
                                    </button>
                                )}
                                {item.status === 'Paid' && (
                                    <span className="px-4 py-2 bg-gray-100 text-gray-400 rounded-xl text-[9px] font-medium border border-gray-200">
                                        Hoàn tất
                                    </span>
                                )}
                            </div>
