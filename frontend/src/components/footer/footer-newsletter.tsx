...
export const FooterNewsletter = ({ brand1, brand2 }: FooterNewsletterProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.data.success) { toast.success(res.data.message || 'Đăng ký thành công!'); setEmail(''); }
            else if (res.data.alreadySubscribed) { toast.error('Email này đã được đăng ký'); }
        } catch (err: any) {
        ...
        <div className="border-b border-gray-800">
            <div className="max-w-[1400px] mx-auto px-4 py-8 flex flex-col lg:flex-row items-center justify-between gap-6">
                <div className="text-center lg:text-left">
                    <h3 className="text-lg font-bold text-white">Đăng ký nhận tin khuyến mãi</h3>
                    <p className="text-gray-500 text-sm mt-1">Nhận deal hot và mã giảm giá từ {brand1} {brand2}.</p>
                </div>
