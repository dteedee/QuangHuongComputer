...
public static class ReportingEndpoints
{
    public static void MapReportingEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/system-health", async (SalesDbContext salesDb) =>
        {
            ...
            try { dbConnected = await salesDb.Database.CanConnectAsync(); }
            ...
            var uptimeHours = (DateTime.UtcNow - process.StartTime.ToUniversalTime()).TotalHours;

            var services = new List<object>
            {
                new { name = "API Server", status = "operational", latency = 0, uptime = $"{uptimeHours:F1}h" },
                new { name = "Database (PostgreSQL)", status = dbConnected ? "operational" : "outage", latency = dbLatencyMs, uptime = dbConnected ? "connected" : "disconnected" }
            };
            ...
            return Results.Ok(new
            {
                ...
                metrics = new { memoryMb, dbLatencyMs, uptimeHours = Math.Round(uptimeHours, 1) },
                services
            });
        });
